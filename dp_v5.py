...
# dp_v5: minimize the depth and cost
        ...
        for l in range(2, n + 1):  # l: length
            for i in range(n - l + 1):
                ...
                for k in range(i, j):
                    ...
                    # print(f'({i}, {j}, {k}, {left.depth}, {right.depth}, {depth})')
                    
                    # if dp_arr[i][j] is None or depth < dp_arr[i][j].depth:
                    if dp_arr[i][j] is None or depth < dp_arr[i][j].depth or (depth == dp_arr[i][j].depth and branch_samples < dp_arr[i][j].branch_samples):
...
for sub_root, count in sub_roots:
    ...
    # only for debug
    if subtree.root.branch_samples() != subtree.dp_arr[0][count].branch_samples:
        raise ValueError('Branch samples not match', subtree.root.branch_samples(), subtree.dp_arr[0][count].branch_samples)

    print(f'cost: {sub_root.branch_samples()}, {subtree.root.branch_samples()}, {subtree.dp_arr[0][count].branch_samples}')
    reduced_cost += sub_root.branch_samples() - subtree.dp_arr[0][count].branch_samples
...
if root.branch_samples() + reduced_cost != sum(samples_list):
    raise ValueError('Branch samples not match', root.branch_samples() + reduced_cost, sum(samples_list))
